...
def parse_strings(template_html, extraction_html):
    ...
    this is useful for testing purposes(mu di)
    """
    t = TokenDict()
...
class InstanceLearningParser(object):
    ...
    def feed(self, html_page):  # feed (wei yang)
        self.html_page = html_page
        # print 'fdsaffffffffffffffassssssssssssss', html_page.__class__
        self.previous_element_class = None
        for index, data in enumerate(html_page.parsed_body):
            if isinstance(data, HtmlTag):
                # print data.start, data.end
                self._add_token(token=data.tag, token_type=data.tag_type, start=data.start, end=data.end)
