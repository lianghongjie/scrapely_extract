# -*- encoding: utf-8 -*-
"""
Region(qu yu) Extract

Custom extraction for regions(qu yu field) in a document
"""
import re
import operator
import copy
...
class RecordExtractor(object):
    ...
    similarity algorithm(suan fa). The annotations are partitioned(fen ge) by the first similar
    region found and searched recursively(di gui).

    Records are represented(biao shi) as dicts mapping attribute names to lists
    containing their values.

    For example:

    # TODO:1.template_make(htmlpage).annotate(filed, data)-->get_template().body[模板加上注释]
    # TODO:2.下面方法解析
    >>> from scrapely.extraction.pageparsing import parse_strings
    >>> template, page = parse_strings( \
    ...
    def __init__(self, extractors, template_tokens):
        ...
        print 'ggggggggggggggggggggggg:',template_tokens
    ...
    def extract(self, page, start_index=0, end_index=None, ignored_regions=None, **kwargs):
        ...
        for k, v in attributes:
            if isinstance(k, six.integer_types):
                ...
                items.append((k, v))
        print 'fsdafasd:---->', variant_ids, variants, items
...
class TemplatePageExtractor(object):
    ...
    def __init__(self, template, extractors):
        self.extractors = extractors
        # print 'fsddddddddddddddddddddddddd',extractors
        # print type(template), 4564564564564
